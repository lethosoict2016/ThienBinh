...
class Payee extends CI_Controller
{
...

	{
		...
		if (TRUE)
		{
			...
			{
				$array_msg = array(
					'msg' => '<i style="color:#fff" class="fa fa-check-circle-o" aria-hidden="true"></i> Account added Successfully',
					'alert' => 'info',
				);
				$this->session->set_flashdata('status', $array_msg);
			}
			...
			{
				$array_msg = array(
					'msg' => '<i style="color:#c00" class="fa fa-exclamation-triangle" aria-hidden="true"></i> Error Account cannot be added',
					'alert' => 'danger',
				);
				$this->session->set_flashdata('status', $array_msg);
			}
